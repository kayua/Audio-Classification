...
Features:
    - Handles multiple species in single recording (e.g., "aedes aegypti,aedes albopictus")
    - Handles multiple sexes in single recording (e.g., "M,M,F" -> "Male-Female")
    - Flexible audio file detection (with or without extension)
    - Multiple duration formats supported:
      * "17 sec 625 ms"
      * "0:00:30"
      * "0:01:15"
    - Comprehensive statistics generation (20+ metrics)
    - Automatic Excel cleanup: removes entries where audio files are not found
    - Creates backup of original Excel file before modifications
    - Saves removed entries to separate Excel file for review
...
class MosquitoAudioOrganizer:
    ...
    def __init__(self, excel_path: str, dataset_path: str, output_path: str):
        ...
        # Track rows to remove from Excel
        self.rows_to_remove = []
    ...
    def load_metadata(self) -> pandas.DataFrame:
        ...
        # Log species distribution (considering multiple species per row)
        all_species = []
        for species_str in df['especie_mosquito']:
            species_list = self.parse_multiple_values(species_str)
            all_species.extend(species_list)

        species_counts = pandas.Series(all_species).value_counts()
        self.logger.info(f"Species distribution (individual specimens):")
        for species, count in species_counts.items():
            self.logger.info(f"  - {species}: {count} specimens")

        # Log sex distribution
        all_sexes = []
        for sex_str in df['sexo_mosquitos']:
            sex_list = self.parse_multiple_values(sex_str)
            all_sexes.extend([s.upper() for s in sex_list])

        sex_counts = pandas.Series(all_sexes).value_counts()
        self.logger.info(f"Sex distribution (individual specimens):")
        for sex, count in sex_counts.items():
            self.logger.info(f"  - {sex}: {count} specimens")
    ...
    @staticmethod
    def parse_multiple_values(value: str) -> List[str]:
        """
    ...
    def parse_duration(duration_str: str) -> float:
        ...
        # Format: "17 sec 625 ms"
        if 'sec' in duration_str.lower():
            total_seconds = 0.0

            # Extract seconds
            if 'sec' in duration_str.lower():
                parts = duration_str.lower().split('sec')
                sec_part = parts[0].strip()
                try:
                    total_seconds += float(sec_part)
                except:
                    pass

                # Extract milliseconds
                if len(parts) > 1 and 'ms' in parts[1]:
                    ms_part = parts[1].replace('ms', '').strip()
                    try:
                        total_seconds += float(ms_part) / 1000.0
                    except:
                        pass

            return total_seconds

        # Format: "0:00:30" or "0:01:15" or "1:23:45"
    ...
    @staticmethod
    def get_sex_label(sex_values: str) -> str:
        """
        Convert sex code(s) to folder label.

        Handles multiple values: "M,M,M" -> "Male"
                                "M,F" -> "Male-Female"
                                "M" -> "Male"

        Args:
            sex_values: Sex code(s) from Excel (M, F, or multiple)
        ...
        sex_list = [s.upper() for s in sex_list]

        # Get unique sexes
        unique_sexes = set(sex_list)

        # Single sex type
        if len(unique_sexes) == 1:
            sex = unique_sexes.pop()
            if sex == 'M':
                return 'Male'
            elif sex == 'F':
                return 'Female'
        ...
        # Multiple sex types - check if it's M and F
        if 'M' in unique_sexes and 'F' in unique_sexes:
            return 'Male-Female'
    ...
    @staticmethod
    def get_species_label(species_values: str) -> str:
        """
        Convert species name(s) to folder label.

        Handles multiple values: "aedes aegypti,aedes aegypti" -> "aedes_aegypti"
                                "aedes aegypti,aedes albopictus" -> "aedes_aegypti-aedes_albopictus"

        Args:
            species_values: Species name(s) from Excel
        ...
        unique_species.sort()  # Sort for consistency

        # Single species
        if len(unique_species) == 1:
            return unique_species[0].replace(' ', '_').replace('/', '_')

        # Multiple species - join with hyphen
        species_labels = [s.replace(' ', '_').replace('/', '_') for s in unique_species]
        return '-'.join(species_labels)
    ...
    def create_folder_structure(self, species: str, sex: str, quantity: int) -> Dict[str, Path]:
        ...
        Handles multiple species and sexes:
        - Single species: "aedes_aegypti"
        - Multiple species: "aedes_aegypti-aedes_albopictus"
        - Single sex: "Male" or "Female"
        - Multiple sexes: "Male-Female"

        Args:
            species: Mosquito species name(s)
            sex: Sex code(s) (M/F or multiple)
            quantity: Number of specimens

        Returns:
            Dictionary with paths for sounds, spectrograms, and histograms folders
        """
        # Get species folder name (handles multiple species)
        species_folder = self.get_species_label(species)

        # Get sex label (handles multiple sexes)
        sex_folder = self.get_sex_label(sex)

        # Count actual number of specimens
        species_list = self.parse_multiple_values(species)
        quantity_value = len(species_list) if species_list else int(quantity)

        # Create quantity folder
        quantity_folder = f"Quantity_{quantity_value}"
    ...
    def collect_statistics(self, file_id: str, species: str, sex: str, quantity: int,
        ...
        Args:
            ...
            species: Mosquito species (single or multiple)
            sex: Sex code(s)
            quantity: Number of specimens
        ...
        # Parse species and sex
        species_list = self.parse_multiple_values(species)
        sex_list = self.parse_multiple_values(sex)
        species_label = self.get_species_label(species)
        sex_label = self.get_sex_label(sex)

        # Determine actual quantity
        actual_quantity = len(species_list) if species_list else int(quantity)

        # Add metadata
        metrics['file_id'] = file_id
        metrics['species'] = species_label
        metrics['species_raw'] = species
        metrics['species_count'] = len(species_list)
        metrics['species_list'] = ','.join(species_list) if species_list else ''
        metrics['sex'] = sex_label
        metrics['sex_raw'] = sex
        metrics['sex_list'] = ','.join(sex_list) if sex_list else ''
        metrics['quantity'] = actual_quantity
        ...
        # Update by_species (use the clean label)
        self.detailed_stats['by_species'][species_label]['count'] += 1
        self.detailed_stats['by_species'][species_label]['total_duration'] += metrics['duration_seconds']
        self.detailed_stats['by_species'][species_label]['total_size_before'] += metrics['original_file_size_bytes']
        self.detailed_stats['by_species'][species_label]['total_size_after'] += metrics['processed_file_size_bytes']
        self.detailed_stats['by_species'][species_label]['durations'].append(metrics['duration_seconds'])

        # Update by_sex (use the label)
        ...
        # Update by_quantity
        quantity_key = f"Quantity_{actual_quantity}"
    ...
    def process_all(self):
        ...
        5. Remove entries with missing audio files from Excel
        ...
        try:
            ...
            # Store original DataFrame for tracking removed rows
            original_df = df.copy()
            ...
            # List to track indices of rows to keep
            indices_to_keep = []
            ...
            for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing audio files"):
                ...
                try:
                    ...
                    self.logger.debug(f"\nProcessing: {file_id}")
                    self.logger.debug(f"  Species raw: {species}")
                    self.logger.debug(f"  Species parsed: {species_list}")
                    self.logger.debug(f"  Species label: {species_label}")
                    self.logger.debug(f"  Sex raw: {sex}")
                    self.logger.debug(f"  Sex parsed: {sex_list}")
                    self.logger.debug(f"  Sex label: {sex_label}")
                    self.logger.debug(f"  Quantity: {len(species_list) if species_list else quantity}")

                    # Find audio file
                    try:
                        input_path = self.find_audio_file(file_id)
                        self.logger.debug(f"  Found file: {input_path.name}")
                    except FileNotFoundError as e:
                        # File not found - mark for removal
                        self.logger.warning(f"  → Audio file not found: {file_id} - Will be removed from Excel")
                        self.stats['not_found'] += 1

                        # Store removed row data
                        row_dict = row.to_dict()
                        row_dict['removal_reason'] = f"Audio file not found: {file_id}"
                        self.rows_to_remove.append(row_dict)

                        # Don't add to indices_to_keep
                        continue

                    # If we get here, file was found - add index to keep list
                    ...
                    self.stats['failed'] += 1
                    # Keep the row in Excel for other types of errors
                    if idx not in indices_to_keep:
                        indices_to_keep.append(idx)

            # Update DataFrame - keep only rows where files were found
            if self.rows_to_remove:
                self.logger.info(f"\n{'=' * 70}")
                self.logger.info(
                    f"Updating Excel file - removing {len(self.rows_to_remove)} entries with missing audio files")
                self.logger.info(f"{'=' * 70}")

                # Create updated DataFrame with only rows to keep
                df_updated = df.loc[indices_to_keep].copy()

                # Save updated Excel file
                self.save_updated_excel(df_updated)
            else:
                self.logger.info("\nNo missing audio files - Excel file remains unchanged")
    ...
    def save_updated_excel(self, df: pandas.DataFrame):
        """
        Save updated Excel file without rows where audio files were not found.
        Creates a backup of the original file first.

        Args:
            df: Updated DataFrame to save
        """
        try:
            # Create backup of original file
            backup_path = self.excel_path.parent / f"{self.excel_path.stem}_backup{self.excel_path.suffix}"
            ...
            if not backup_path.exists():
                ...
                self.logger.info(f"Backup already exists: {backup_path}")

            # Save updated Excel file
            df.to_excel(self.excel_path, index=False)
            self.logger.info(f"Updated Excel file saved: {self.excel_path}")
            self.logger.info(f"Removed {len(self.rows_to_remove)} rows with missing audio files")

            # Also save a separate file with removed entries
            if self.rows_to_remove:
                removed_df = pandas.DataFrame(self.rows_to_remove)
                removed_path = self.output_path / 'removed_entries.xlsx'
                removed_df.to_excel(removed_path, index=False)
                self.logger.info(f"Removed entries saved to: {removed_path}")

        except Exception as e:
            self.logger.error(f"Error saving updated Excel file: {e}")
            raise
    ...
    def _print_summary(self):
        ...
        self.logger.info(f"Not found (removed):    {self.stats['not_found']}")
        self.logger.info(f"Failed (other errors):  {self.stats['failed']}")
        ...
        if self.stats['not_found'] > 0:
            self.logger.info(f"  - {self.output_path / 'removed_entries.xlsx'} (files not found)")
