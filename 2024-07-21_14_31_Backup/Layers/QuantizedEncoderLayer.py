    ...
    def call(self, inputs, training=False):
        ...
        Returns:
            tuple: A tuple of two tensors:
                - `neural_network_flow`: Output tensor after applying convolution, pooling, and dense layer.
                - `neural_network_flow_quantized`: Output tensor after quantization and dropout.
        """
        ...
        neural_network_flow_quantized = self.quantize_layer(neural_network_flow)

        if training:
            random_mask = tf.random.uniform(tf.shape(neural_network_flow_quantized), 0, 1)
            neural_network_flow_quantized = neural_network_flow_quantized * random_mask
            neural_network_flow_quantized = self.dropout(neural_network_flow_quantized)

        return neural_network_flow, neural_network_flow_quantized
