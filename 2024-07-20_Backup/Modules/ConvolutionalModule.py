...
class ConvolutionalModule(Layer):
    ...
    def __init__(self,
        ...
        **kwargs : Additional keyword arguments.
        ...
        self.depth_wise_convolutional = Conv1D(number_filters, size_kernel, groups=number_filters,
                                               padding=self.convolutional_padding)
        self.batch_normalization = BatchNormalization()
        self.swish_activation = Activation(tensorflow.nn.swish)
        self.second_point_wise_convolutional = Conv1D(number_filters, kernel_size=1)
    ...
    def call(self, neural_network_flow: tensorflow.Tensor) -> tensorflow.Tensor:
        ...
        # Ensuring the shape is correct for Conv1D
        shape = tensorflow.shape(neural_network_flow)
        batch_size, seq_len, channels = shape[0], shape[1], shape[2]
        neural_network_flow = tensorflow.reshape(neural_network_flow, (batch_size, seq_len, channels))
