...
class ConvNeXtGradientMaps:
    ...
    def build_gradcam_model(self, target_layer_name: str = None) -> None:
        ...
        Args:
            target_layer_name: Name of target layer. If None, uses last stage's last block
        """
        if self.neural_network_model is None:
            raise ValueError("Model must be built before creating GradCAM model")

        # Find last stage's last block if no target specified
        if target_layer_name is None:
            # Get last stage's last block
            last_stage_idx = len(self.depths) - 1
            last_block_idx = self.depths[last_stage_idx] - 1
            target_layer_name = f'convnext_block_stage_{last_stage_idx}_block_{last_block_idx}'
